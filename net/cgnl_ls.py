...
class SpatialCGNL(nn.Module):
    ...
    """
    def __init__(self, inplanes, planes, use_scale=False, groups=None, flag='short'):
    ...
    def forward(self, x):
        ...
        if self.groups and self.groups > 1:
            if self.flag == 'short':
                ...
                _c = int(zs / self.groups)
                ts = torch.split(t, split_size_or_sections=self.groups, dim=2)
                ps = torch.split(p, split_size_or_sections=self.groups, dim=2)
                gs = torch.split(g, split_size_or_sections=self.groups, dim=2)
                _t_sequences = []
                for i in range(self.groups):
                    t_long = []
                    p_long = []
                    g_long = []
                    for j in range(_c):
                        t_long.append(ts[j][:, :, i, :, :].unsqueeze(2))
                        p_long.append(ps[j][:, :, i, :, :].unsqueeze(2))
                        g_long.append(gs[j][:, :, i, :, :].unsqueeze(2))
                    t_long_s = torch.cat(t_long, dim=2)
                    p_long_s = torch.cat(p_long, dim=2)
                    g_long_s = torch.cat(g_long, dim=2)
                    _x = self.kernel(t_long_s, p_long_s, g_long_s,
                                     b, c, _c, h, w)
                    _t_sequences.append(_x)
                x = torch.cat(_t_sequences, dim=2)
        ...
        if self.flag == 'short' or self.groups == 1:
            x = self.z(x)
            ...
            x = self.gn(x)
            x_new = []
            _t_sequences = torch.split(x, split_size_or_sections=_c, dim=2)
            for j in range(_c):
                for i in range(self.groups):
                    x_new.append(_t_sequences[i][:, :, j, :, :].unsqueeze(2))
            x = torch.cat(x_new, dim=2) + residual

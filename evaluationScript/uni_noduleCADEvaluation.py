...
seriesuid_label = 'series_id'
coordX_label = 'x_center'
coordY_label = 'y_center'
coordZ_label = 'z_center'
w_label = 'w_mm'
h_label = 'h_mm'
d_label = 'd_mm'
    ...
    for seriesuid in seriesUIDs:
        ...
        for result in results[1:]:
            ...
            if seriesuid == nodule_seriesuid:
                nodule = getCanNodule(result, header)
        ...
        if (maxNumberOfCADMarks > 0):
            ...
            if len(nodules.keys()) > maxNumberOfCADMarks:
                ...
                probThreshold = probs[maxNumberOfCADMarks] - 1e-8
                nodules2 = {}
                nrNodules2 = 0
                for keytemp, noduletemp in nodules.items():
...
def getCanNodule(annotation, header, state = ""):
    ...
    if w_label in header:
        nodule.w_mm = annotation[header.index(w_label)]
        nodule.h_mm = annotation[header.index(h_label)]
        nodule.d_mm = annotation[header.index(d_label)]
        # nodule.diameter_mm = str(pow((pow((float(nodule.w_mm) / 2.0), 2.0) + pow((float(nodule.h_mm) / 2.0), 2.0) + pow((float(nodule.d_mm) / 2.0), 2.0)), 0.5) * 2)
        # nodule.diameter_mm = str((float(nodule.w_mm) + float(nodule.h_mm) + float(nodule.d_mm)) / 3)
        nodule.diameter_mm = str(max(float(nodule.w_mm), float(nodule.h_mm), float(nodule.d_mm)))
...
def getNodule(annotation, data_dir, seriesuid, header, state=""):
    nodule = NoduleFinding()
    nodule.id = annotation[header.index(seriesuid_label)]
    if coordX_label in header:
        ...
        # lungbox_origin = np.load(os.path.join(data_dir, 'preprocessed/%s_lungbox_origin.npy' % (seriesuid)))
        # spacing = np.load(os.path.join(data_dir, 'preprocessed/%s_spacing.npy' % (seriesuid)))
    ...
    if diameter_mm_label in header:
        ...
        nodule.h_mm = nodule.diameter_mm
        nodule.d_mm = nodule.diameter_mm
    ...
    if xmin_label in header:
        ...
        nodule.w_mm = str(float(nodule.xmax) - float(nodule.xmin) + 1)
        nodule.h_mm = str(float(nodule.ymax) - float(nodule.ymin) + 1)
        nodule.d_mm = str(float(nodule.zmax) - float(nodule.zmin) + 1)
        # nodule.w_mm = str(float(nodule.xmax) - float(nodule.xmin))
        # nodule.h_mm = str(float(nodule.ymax) - float(nodule.ymin))
        # nodule.d_mm = str(float(nodule.zmax) - float(nodule.zmin))
        # nodule.diameter_mm = str((float(nodule.w_mm) + float(nodule.h_mm) + float(nodule.d_mm)) / 3)
        nodule.diameter_mm = str(max(float(nodule.w_mm), float(nodule.h_mm), float(nodule.d_mm)))
...
def noduleCADEvaluation(annotations_filename,data_dir,seriesuids_filename,results_filename,outputDir):
    ...
    (allNodules, seriesUIDs) = collect(annotations_filename, data_dir, seriesuids_filename)
